...
* @param {Number} [opts.lambda=1] - shape parameter
* @param {Number} [opts.k=1] - scale parameter
...
function quantile( p, options ) {
	...
	if ( isNumber( p ) ) {
		return quantile1( p, opts.lambda, opts.k );
	}
	if ( isMatrixLike( p ) ) {
		if ( opts.copy !== false ) {
		...
		return quantile5( out, p, opts.lambda, opts.k );
	}
	if ( isTypedArrayLike( p ) ) {
		...
		return quantile6( out, p, opts.lambda, opts.k );
	}
	if ( isArrayLike( p ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return quantile4( p, opts.lambda, opts.k, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return quantile3( out, p, opts.lambda, opts.k, opts.accessor );
		}
		return quantile2( out, p, opts.lambda, opts.k );
	}
	return NaN;
