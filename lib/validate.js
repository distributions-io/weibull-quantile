...
* @param {Number} [options.lambda] - shape parameter
* @param {Number} [options.k] - scale parameter
...
function validate( opts, options ) {
	...
	if ( options.hasOwnProperty( 'lambda' ) ) {
		...
		if ( !isPositive( opts.lambda ) ) {
			return new TypeError( 'quantile()::invalid option. `lambda` parameter must be a positive number. Option: `' + opts.lambda + '`.' );
		}
	}
	if ( options.hasOwnProperty( 'k' ) ) {
		opts.k = options.k;
		if ( !isPositive( opts.k ) ) {
			return new TypeError( 'quantile()::invalid option. `k` parameter must be a positive number. Option: `' + opts.k + '`.' );
		}
	}
